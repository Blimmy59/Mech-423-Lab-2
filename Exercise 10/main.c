...
volatile unsigned int counter = 0; //depicts how many bytes in the buffer
volatile unsigned int command = 0;
volatile unsigned int top = 0;
volatile unsigned int bottom = 0;
volatile unsigned int esc = 0;
...
int main(void)
{
    ...
    while (1)                             //infinite loop
    {
        if (counter > 4)
        {
            while(queue[start] != 0xFF && counter > 0)
            {
                start++;
                counter--;
            }
            if (queue[start] == 0xFF)
            {
                ...
                {
                    //GET DATA BYTES
                    top = queue[start]; //pop front
                    start++;
                    counter--;

                    bottom = queue[start]; //pop front
                    start++;
                    counter--;

                    esc = queue[start]; //pop front
                    start++;
                    counter--;
                    if(esc == 0x1)
                        bottom = 0xFF;
